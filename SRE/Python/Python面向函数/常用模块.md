...
shutil.copyfileobj(open('a.txt','r'),open('a.txt.new','w')) # 拷贝内容

shutil.copyfile('file.txt','file1.txt') # 拷贝文件

shutil.copystat('file.txt','file1.txt') # 拷贝文件信息
...
shutil.mkdir(directory_path)    # 创建单个目录

shutil.makedirs(directory_path) # 创建嵌套目录
```
