...
if __name__ == "__main__":
    ...
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)  # 端口复用 [3,4](@ref)
...
if __name__ == "__main__":
    start = time.time()
    with Pool(4) as pool:   # context管理：自动调用 pool.close() and pool.join() 
        results = []
        for i in range(4):
...
if __name__ == '__main__':
    start = time.time()
    with Pool(4) as pool:
        async_results = [pool.apply_async(task, (i,)) for i in range(4)]  # 异步提交
