...
通过实际操作来配置 Redis 访问控制、启用 SSL 加密，并实施完整的安全策略。
...
# 创建只读用户
redis-cli -a "$STRONG_PASSWORD" ACL SETUSER readonly on ">readonly123" "~*" "&*" "-@all" "+@read"
echo "只读用户创建完成"

# 创建应用用户
redis-cli -a "$STRONG_PASSWORD" ACL SETUSER appuser on ">app123" "~app:*" "&*" "-@all" "+get" "+set" "+del" "+exists" "+expire"
echo "应用用户创建完成"

# 创建管理用户
redis-cli -a "$STRONG_PASSWORD" ACL SETUSER admin on ">admin123" "~*" "&*" "+@all"
echo "管理用户创建完成"

# 查看用户列表
echo "当前用户列表:"
redis-cli -a "$STRONG_PASSWORD" ACL LIST
...
# 测试只读用户
echo "测试只读用户权限:"
redis-cli --user readonly --pass readonly123 SET test_key "test_value" 2>&1 | grep -q "NOPERM" && echo "✓ 只读用户无法写入" || echo "✗ 只读用户权限异常"
redis-cli -a "$STRONG_PASSWORD" SET test_key "test_value"
redis-cli --user readonly --pass readonly123 GET test_key > /dev/null && echo "✓ 只读用户可以读取" || echo "✗ 只读用户无法读取"

# 测试应用用户
echo "测试应用用户权限:"
redis-cli --user appuser --pass app123 SET app:config "value" > /dev/null && echo "✓ 应用用户可以写入app:*键" || echo "✗ 应用用户无法写入"
redis-cli --user appuser --pass app123 SET global:config "value" 2>&1 | grep -q "NOPERM" && echo "✓ 应用用户无法写入其他键" || echo "✗ 应用用户权限异常"
...
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
    -keyout redis.key \
    -out redis.crt \
    -subj "/C=CN/ST=Beijing/L=Beijing/O=Redis/CN=localhost" 2>/dev/null

if [ -f "/etc/redis/ssl/redis.crt" ]; then
    echo "✓ SSL证书生成成功"
    sudo chown redis:redis /etc/redis/ssl/* 2>/dev/null || echo "注意：请确保Redis用户权限"
else
    echo "✗ SSL证书生成失败"
fi
...
class EncryptedRedis:
    def __init__(self, password, key=None):
        self.redis = redis.Redis(host='localhost', port=6379, password=password)
        self.key = key or Fernet.generate_key()
        self.cipher = Fernet(self.key)
        print(f"加密密钥: {self.key.decode()}")
    
    def set_encrypted(self, name, value):
        if isinstance(value, dict):
            value = json.dumps(value)
        encrypted_value = self.cipher.encrypt(value.encode())
        return self.redis.set(name, encrypted_value)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("用法: python3 redis_encrypt.py <redis_password>")
        sys.exit(1)
    
    password = sys.argv[1]
    encrypted_redis = EncryptedRedis(password)
    ...
    encrypted_redis.set_encrypted("sensitive:user:1", sensitive_data)
    print("敏感数据已加密存储")
    
    # 读取解密数据
    decrypted_data = encrypted_redis.get_decrypted("sensitive:user:1")
    print(f"解密数据: {decrypted_data}")
EOF

echo "数据加密脚本已创建: /tmp/redis_encrypt.py"
if command -v python3 >/dev/null && python3 -c "import redis, cryptography" 2>/dev/null; then
    python3 /tmp/redis_encrypt.py "$STRONG_PASSWORD"
else
    echo "请安装 redis-py 和 cryptography 库来运行加密示例"
fi
...
# 检查密码
echo "检查密码配置:"
if redis-cli -a "$1" CONFIG GET requirepass | grep -q "requirepass"; then
    echo "✓ 密码认证已启用"
else
    echo "✗ 密码认证未启用"
fi
...
# 检查ACL
echo "检查ACL配置:"
USER_COUNT=$(redis-cli -a "$1" ACL LIST | wc -l)
if [ $USER_COUNT -gt 1 ]; then
    echo "✓ ACL用户已配置 ($USER_COUNT 个用户)"
else
    echo "✗ 仅有默认用户"
fi
...
# 检查绑定地址
echo "检查网络绑定:"
BIND_ADDR=$(redis-cli -a "$1" CONFIG GET bind | tail -1)
echo "绑定地址: $BIND_ADDR"

# 检查保护模式
echo "检查保护模式:"
PROTECTED=$(redis-cli -a "$1" CONFIG GET protected-mode | tail -1)
echo "保护模式: $PROTECTED"
...
/tmp/security_check.sh "$STRONG_PASSWORD"

# 7. 创建安全监控脚本
echo "7. 创建安全监控脚本"

cat > /tmp/redis_security_monitor.sh << 'EOF'
#!/bin/bash
# Redis 安全监控脚本

LOG_FILE="/tmp/redis_security.log"
REDIS_PASSWORD="$1"

if [ -z "$REDIS_PASSWORD" ]; then
    echo "用法: $0 <redis_password>"
    exit 1
fi

echo "$(date): 开始安全监控" >> $LOG_FILE

# 监控连接数
CONNECTIONS=$(redis-cli -a "$REDIS_PASSWORD" INFO clients | grep connected_clients | cut -d: -f2 | tr -d '\r')
echo "$(date): 当前连接数: $CONNECTIONS" >> $LOG_FILE

# 监控慢查询
SLOW_COUNT=$(redis-cli -a "$REDIS_PASSWORD" SLOWLOG LEN)
if [ $SLOW_COUNT -gt 0 ]; then
    echo "$(date): 发现慢查询: $SLOW_COUNT 条" >> $LOG_FILE
    redis-cli -a "$REDIS_PASSWORD" SLOWLOG GET 5 >> $LOG_FILE
fi
...
if [ -f "/var/log/redis/redis-server.log" ]; then
    FAILED_AUTH=$(grep "Wrong password" /var/log/redis/redis-server.log | tail -5)
    if [ ! -z "$FAILED_AUTH" ]; then
        echo "$(date): 发现认证失败尝试" >> $LOG_FILE
        echo "$FAILED_AUTH" >> $LOG_FILE
    fi
fi
...
chmod +x /tmp/redis_security_monitor.sh
echo "安全监控脚本已创建: /tmp/redis_security_monitor.sh"

# 运行一次监控
/tmp/redis_security_monitor.sh "$STRONG_PASSWORD"
echo "监控结果:"
cat /tmp/redis_security.log

# 8. 备份安全配置
echo "8. 备份安全配置"

# 导出ACL配置
redis-cli -a "$STRONG_PASSWORD" ACL SAVE
echo "ACL配置已保存"

# 导出当前配置
redis-cli -a "$STRONG_PASSWORD" CONFIG GET '*' > /tmp/redis_secure_config.txt
echo "当前配置已导出到: /tmp/redis_secure_config.txt"

# 9. 生成安全报告
echo "9. 生成安全配置报告"

cat > /tmp/redis_security_report.md << EOF
# Redis 安全配置报告
...
1. **readonly**: 只读权限，可访问所有键
2. **appuser**: 应用用户，仅可访问 app:* 键模式
...
- 加密脚本: /tmp/redis_encrypt.py
- 安全检查: /tmp/security_check.sh
- 监控脚本: /tmp/redis_security_monitor.sh
- 配置备份: /tmp/redis_secure_config.txt
- 监控日志: /tmp/redis_security.log

生成时间: $(date)
EOF

echo "安全配置报告已生成: /tmp/redis_security_report.md"
...
echo ""
echo "重要文件和脚本:"
echo "- 安全报告: /tmp/redis_security_report.md"
echo "- 监控脚本: /tmp/redis_security_monitor.sh"
echo "- 加密工具: /tmp/redis_encrypt.py"
echo "- 配置备份: /tmp/redis_secure_config.txt"

echo ""
echo "Redis管理员密码: $STRONG_PASSWORD"
echo "请妥善保管密码，并定期更换！"
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    redis-cli -a "$STRONG_PASSWORD" DEL test_key app:config sensitive:user:1
    echo "测试数据已清理"
else
    echo "保留测试数据"
fi
...
1. **访问控制**：配置密码认证、ACL用户管理和权限控制
2. **网络安全**：实施端口安全、SSL/TLS加密和防火墙配置
3. **数据安全**：应用数据加密、敏感数据处理和安全备份
4. **安全监控**：部署审计日志和安全监控系统
5. **最佳实践**：实施安全配置检查、漏洞防护和应急响应
6. **实践应用**：通过实际操作建立完整的Redis安全防护体系

Redis 安全管理是一个持续的过程，需要根据业务需求和威胁环境不断调整和完善安全策略。定期的安全评估和更新是保障 Redis 安全运行的关键。