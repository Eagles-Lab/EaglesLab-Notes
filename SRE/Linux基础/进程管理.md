...
![image-20240726133933861](进程管理\管道通信.png)
...
![image-20240725152839874](进程管理\共享内存.png)
...
![image-20240725152225317](进程管理\信号含义及其对应的值.png)
...
![image-20230411100858598](进程管理\套接字.png)
...
* **T**: 暂停执行
* **Z**: 不存在但暂时无法消除，也叫僵尸进程
  * 每个进程在运行结束后都会处于僵死状态，等待父进程调用进而释放系统资源，处于该状态的进程已经运行结束，但是它的父进程还没有释放其系统资源，即**僵尸进程是一个早已死亡的进程，但在进程表中仍占据一个位置**
  * 孤儿进程：如果父进程先结束，子进程就会变成孤儿进程，会被 init 进程接管，子进程结束后其占用资源就会被 init 进程回收
* **W**: 没有足够的内存可分配
* **<**: 高优先序的进程
* **N**: 低优先序的进程
* **+**：前台进程
